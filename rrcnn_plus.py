...
from RPConv1DBlock_attention import RPConv1DBlock_attention
import numpy as np
import pandas as pd
import sys,pywt
...
def create_model(x_train, y_train, x_test, y_test):
    ...
    ## cell 1
    outs = RPConv1DBlock_attention({{choice(range(5, 100, 5))}})(inputs)
    outs = RPConv1DBlock_attention({{choice(range(5, 100, 5))}})(outs)
    outs = RPConv1DBlock_attention({{choice(range(5, 100, 5))}})(outs)
    outs = RPConv1DBlock_attention({{choice(range(5, 100, 5))}})(outs)
    outs = RPConv1DBlock_attention({{choice(range(5, 100, 5))}})(outs)
    outs = RPConv1DBlock_attention({{choice(range(5, 100, 5))}})(outs)
    ...
    ## cell 1
    inputs2 = inputs - outs
    outs = RPConv1DBlock_attention({{choice(range(10, 150, 5))}})(inputs2)
    outs = RPConv1DBlock_attention({{choice(range(10, 150, 5))}})(outs)
    outs = RPConv1DBlock_attention({{choice(range(10, 150, 5))}})(outs)
    outs = RPConv1DBlock_attention({{choice(range(10, 150, 5))}})(outs)
    outs = RPConv1DBlock_attention({{choice(range(10, 150, 5))}})(outs)
    outs = RPConv1DBlock_attention({{choice(range(10, 150, 5))}})(outs)
    ...
    outs = RPConv1DBlock_attention({{choice(range(10, 150, 5))}})(outs)
    outs = RPConv1DBlock_attention({{choice(range(10, 150, 5))}})(outs)
    outs = TVD_IC(0.04, 5)(outs)
    outs1 = ExtMidSig(x_train.shape[1], int(y_train.shape[1]/2))(outs)
    
    ## cell 2
    inputs3 = inputs2 - outs
    outs = RPConv1DBlock_attention({{choice(range(10, 150, 5))}})(inputs3)
    outs = RPConv1DBlock_attention({{choice(range(10, 150, 5))}})(outs)
    outs = RPConv1DBlock_attention({{choice(range(10, 150, 5))}})(outs)
    outs = RPConv1DBlock_attention({{choice(range(10, 150, 5))}})(outs)
    outs = RPConv1DBlock_attention({{choice(range(10, 150, 5))}})(outs)
    ...
    # Train
    model.fit(x=x_train, y=y_train, epochs={{choice(range(20,60,10))}}, 
                batch_size=8, verbose=1, validation_data=(x_test, y_test))
    
    pred = model.predict(x_test)
    score = mean_squared_error(pred[:,:,0], y_test[:, :,0])
    weight_path = './models/model_dataset2_rrcnnplus.h5'
    #try:
    #    with open('rrcnn_manually_imporve.txt') as f:
    ...
    if score <= min_error:
        ...
        min_error = score
    #    with open('rrcnn_manually_improve.txt','w') as f:
...
if __name__ == '__main__':
    ...
    best_model = load_model(weight_path, \
            custom_objects={'ReflectionPadding1D':ReflectionPadding1D, 'RPConv1DBlock_attention':RPConv1DBlock_attention, 'ExtMidSig':ExtMidSig, 'CustomTV_MSE':CustomTV_MSE, 'TVD_IC':TVD_IC}) 
